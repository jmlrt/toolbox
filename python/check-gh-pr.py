...
def get_data(repository):
    response = subprocess.check_output(['curl', '-s', 'https://api.github.com/search/issues?q=repo:{}+is:pr+is:open&per_page=100'.format(repository)])
...
def main():
    ...
    parser_top_authors = subparsers.add_parser('top-authors', help='get the top 5 of pull requests authors')
    parser_top_authors.set_defaults(func=top_authors)

    parser_list_pull_requests = subparsers.add_parser('list-pull-requests', help='list pull requests sorted by authors')
